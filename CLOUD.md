...
- `light` → 最近收盤已被前一份涵蓋（週末、假日、或今天稍早已跑過）→ **只做輕量更新**：
  ...
  不做行情抓取、不重算情境、不做個股篩選。
  **標題日期與 `資料基準日` 屬性一律沿用 `trading_day.py` 回傳的 `last_completed_session`（上一份已涵蓋的交易日），不得填今天的執行日**——
  即使今天執行、macro/新聞資料是今天抓的，頁面代表的仍是同一個交易日的補充更新，不是新的一個交易日。
  執行日期只放在內文「本次更新時間」一行，不進標題或屬性。這個屬性值也是健康檢查比對的依據，填錯會觸發誤報。
