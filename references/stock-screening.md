# 題材與台股篩選規則

先篩題材，再篩個股。熱門、漲停、成交量放大或新聞頻繁不是單獨的入選理由。

## 一、題材傳導鏈

為每個題材寫出完整鏈條：

```text
總經／政策／新聞驅動
  → 需求、價格、利率、匯率、成本或供給改變
  → 產業營收、毛利、資本支出或資產負債表影響
  → 具直接曝險的公司
  → 可觀察催化劑與失效條件
```

鏈條中任何一步沒有證據時，降低分數；只有同產業標籤、沒有實質曝險時不得視為直接受益。

## 二、題材評分

先計算 100 分原始分，再扣 0–30 分風險折減：

| 維度 | 配分 | 高分條件 |
|---|---:|---|
| 證據強度 | 20 | 多個 A/B 級來源、資料時間一致 |
| 傳導清晰度 | 20 | 可量化連結需求／價格／成本到獲利 |
| 廣度與持續性 | 15 | 不只一次性事件，戰術與策略期均有依據 |
| 政策確定性 | 15 | 已通過或已生效，範圍與時程明確 |
| 企業基本面確認 | 15 | 營收、訂單、法說或財報已有支持 |
| 估值與擁擠度 | 10 | 尚未完全反映，籌碼或估值不極端 |
| 催化劑可驗證性 | 5 | 有日期、數值或事件可追蹤 |

風險折減包括：政策仍為提案、地緣尾端風險、原料或匯率反向曝險、過度擁擠、估值極端、資料過時、因果關係脆弱。

題材狀態：

- 淨分 `≥75`：`PRIORITY_RESEARCH`，但至少要有一個 A/B 級驅動來源與一個公司層級確認。
- 淨分 `60–74`：`WATCH`。
- 淨分 `<60`：`ABSTAIN`。
- 缺少政策狀態、公司受益機制或失效條件：不論分數最高只能 `WATCH`。

## 三、股票池與硬性排除

### 兩階段篩選

1. **第一階段（全市場粗篩）**：`python scripts/screen_universe.py --top 30 --core 2330,2317,2454,2382,2408,2344,3711,2308 --output <shortlist.json>`
   一次 BWIBBU_ALL + STOCK_DAY_ALL 呼叫，掃全部上市普通股，粗篩（單日成交金額 ≥ 5,000 萬、0<PER≤60、排除跌停），依「成交量排名 + 當日振幅 + 輕度低本益比」混合分排序取前 30，再加固定核心清單。輸出 `codes`。
2. **第二階段（深度資料）**：對 `shortlist.json` 的 `codes` 跑 `fetch_fundamentals.py` 與 watchlist 版 `fetch_market_snapshot.py`，取得 PER 分位、法人、月營收、訊號。
3. 題材與個股評分仍**從題材出發**：先確定優先題材，再從 shortlist ∪ 核心清單中找該題材的直接曝險標的。shortlist 是「網」，不是候選名單本身——它讓固定 8 檔以外的輪動標的有機會被看見。

FinMind 呼叫量：shortlist 30 檔 × 5 dataset ＝ 150 次/日，仍在免費額度內；超過就縮 `--top`。

### 硬性排除

預設股票池先以分析截止時間當下、代號首位為 1–9 的四位數 TWSE／TPEx 證券作為普通股代理，再以交易所或公司資料確認工具類型。執行以下硬性排除：

- 非普通股、ETF、ETN、權證、債券或無法確認工具類型。
- 停止交易、處置狀態嚴重限制可執行性、無可靠收盤價或資料日期不一致。
- 流動性未達門檻（見下方兩段式規則）。
- 無法從公司公告、財報、法說或營收結構證明題材曝險。
- 最新財務資訊超過合理發布週期，或存在重大持續經營、治理、財報可靠性疑慮而未被充分說明。

### 流動性門檻（兩段式，依可得資料）

1. **有多日歷史**（`fetch_market_snapshot.py` 的 `watchlist.matches[].liquidity_history.available = true`，或使用者提供含歷史的檔案）：
   近 20 交易日中位成交金額 < 新台幣 2,000 萬元 → 硬性排除。`sessions_below_20m` 佔比高（≥5 日）→ 風險折減。
2. **只有單日快照**（一般情況）：
   用當日成交金額作代理 — < 新台幣 2,000 萬元 → 硬性排除；新台幣 2,000 萬–1 億元 → 保留但流動性維度信心標 `低`、扣風險折減，且個股狀態最高只能 `WATCH`。
   不要因為「沒有 20 日資料」就把所有標的標成資料不足；只有進入前幾名的候選才值得用 `--watchlist` 重跑腳本補抓歷史。

最終候選（`PRIORITY_RESEARCH`）必須有多日歷史佐證流動性；達不到就降為 `WATCH`。

流動性門檻可依使用者資金規模提高，但不得在未詢問或未取得資金規模時宣稱適合其個人交易。

## 四、個股評分

計算 100 分原始分，再扣 0–30 分風險折減。**資料來源優先用腳本輸出**：`market.json` 的 `watchlist.matches[].signals`、`fundamentals.json` 的 `codes[].*`。

| 維度 | 配分 | 所需證據與資料來源 |
|---|---:|---|
| 題材直接曝險 | 22 | 產品、地區、客戶、產能或營收占比（公司公告／法說／營收結構） |
| 營運與獲利確認 | 18 | `month_revenue`：latest YoY／MoM、`trailing_3m_avg_yoy_pct`；加訂單、毛利、展望。月營收 3 月均 YoY 明顯放緩 → 扣分 |
| 財務品質 | 12 | 現金流、槓桿、資本支出、治理、股利（`dividend`） |
| 估值 | 15 | `valuation_history`：`per_1y_percentile`、`pbr_1y_percentile`、1 年 PER 區間。分位 ≤30 且獲利成長 → 高分；分位 ≥90 → 低分且入風險折減；無 FinMind 資料退回當日 PER／PBR 並降信心 |
| 籌碼面 | 10 | `institutional`：三大法人 5／20 日淨買超方向與規模、`trust_net_20d`；`margin_short`：融資餘額 5／20 日變化（急增為負向）、券資比 |
| 市場驗證（量化） | 10 | `signals`：`rel_strength_20d_pct`（相對大盤強弱）、`return_20d/60d_pct`、`volume_ratio_vs_20d`、`vs_ma20/60_pct`。只作驗證，不作單獨入選理由 |
| 流動性與可執行性 | 8 | 20 日中位成交金額、交易狀態、事件風險 |
| 催化劑 | 10 | 財報、法說、投產、訂單、政策、除息（`dividend.cash_ex_date`）時間點 |

風險折減包括：客戶集中、單一產品、匯率與原料敏感、負債、稀釋、治理、法規、地緣、景氣反轉、**PER 近一年分位 ≥90**、**融資餘額 20 日急增**、**近月營收 MoM 大幅轉弱**、題材已高度反映。

個股狀態：

- 淨分 `≥75`、沒有硬性排除、**且 `valuation_history.available` 為真（有近一年估值分位佐證）**：`PRIORITY_RESEARCH`。
- 淨分 `60–74`，或分數達標但無 FinMind 估值分位：`WATCH`。
- 淨分 `<60`、硬性排除或關鍵資料缺失：`ABSTAIN`。

即使分數達標，沒有經驗證的交易策略、成本估計與前向證據時也不得標為 `BUY`。

### 評分量表（rubric，對照打分而非估分）

每個維度依下表給 0／1／2／3，再乘配分權重（權重 ÷ 3）。缺關鍵資料 → 該維度最高 1。

| 維度 | 0 | 1 | 2 | 3 |
|---|---|---|---|---|
| 題材直接曝險 | 只有同產業標籤 | 有曝險但佔比小或未量化 | 明確產品／客戶／產能曝險，佔營收可估 | 該題材為公司主要成長引擎，佔比高且公司自述 |
| 營運與獲利確認 | 月營收 YoY 轉負或缺資料 | YoY 個位數、動能持平 | YoY 雙位數、3 月均正 | YoY 高雙位數且 MoM、法說同向 |
| 財務品質 | 有治理／持續經營疑慮 | 槓桿偏高或現金流弱 | 財務穩健、正常配息 | 淨現金、現金流強、股利政策明確 |
| 估值 | PER 近一年分位 ≥90 | 分位 70–89 | 分位 40–69 | 分位 ≤39 且獲利成長 |
| 籌碼面 | 三大法人 20 日淨賣 + 融資急增 | 法人中性、融資平穩 | 法人 20 日淨買、融資未膨脹 | 外資＋投信同步 20 日淨買、券資比健康 |
| 市場驗證 | 相對大盤 20 日弱 >5% 或量縮動能衰竭 | 中性、與大盤同步 | 相對大盤略強、站上 MA20 | 相對大盤明顯強、量價齊揚但未過熱 |
| 流動性 | 無 20 日資料或單日 <2,000 萬 | 20 日中位 2,000 萬–1 億 | 20 日中位 1–10 億 | 20 日中位 >10 億 |
| 催化劑 | 無可追蹤催化劑 | 催化劑模糊、無日期 | 有明確日期或數值可追蹤 | 多個近期催化劑、時程明確 |

原始分 ＝ Σ(各維度分 × 權重 ÷ 3)。風險折減與狀態門檻不變。

### 量化訊號的解讀（`signals`）

- `rel_strength_20d_pct` 明顯為正 = 近月跑贏大盤；明顯為負 = 落後（如題材股卻落後，是警訊）。
- `return_60d_pct` 已大幅為正 + `vs_ma20_pct` 高 = 漲多，追價風險，估值維度要更嚴。
- `volume_ratio_vs_20d` ≫ 1 + 大漲 = 過熱訊號；≪ 1 = 量縮，動能可能衰竭。
- `realized_vol_20d_annual_pct` 極高（如 >50%）= 波動大，部位與失效條件要更保守。
- 訊號互相矛盾（漲但法人賣、量縮）→ 在研究摘要點出，降市場驗證分。

## 五、候選數量與多樣性

- 每個題材最多列 5 檔，優先列 2–3 檔證據最完整者。
- 不因題材熱門而列滿名額。
- 同一集團、同一高度相關供應鏈若造成重複曝險，合併解釋集中風險。
- 至少列一個未入選但市場敘事熱門的標的，說明排除原因。

## 六、每檔候選必填欄位

```text
code
company
theme
analysis_as_of
evidence_date
direct_exposure
fundamental_confirmation      (含月營收 YoY/MoM 與 3 月均 YoY)
valuation_basis               (PER 近一年分位 + 1 年區間；無資料則當日 PER/PBR + 降信心)
chip_side                     (三大法人 5/20 日淨買超、融資餘額變化、券資比)
quant_signals                 (相對大盤 20 日強弱、20/60 日動能、量能比)
liquidity_evidence            (註明是 20 日中位或單日代理值)
liquidity_window_days
catalysts                     (含除息日)
counter_evidence
invalidation_conditions
gross_score
risk_deduction
net_score
status
change_vs_previous            (新進榜/維持/升級/降級 + 原因)
source_urls
```

資料缺少時填 `N/A`；不得用模糊形容詞取代估值、流動性或曝險證據。
