...
### 頁面標題
直接用 `scripts/trading_day.py` 輸出的 `report_title`（已含 `(輕量)`／`(修訂 N)` 後綴），不要自己重組標題字串或另外判斷日期。

### 若父層是資料庫，對應屬性（存在才填，不存在不強制建立）
| 屬性 | 值 |
|---|---|
| 日期 / Date | `trading_day.py` 的 `notion_date_property`（直接照抄，不要自己重新判斷日期） |
| 狀態 / Status | `scripts/summarize_run_status.py` 的 `suggested_report_status`（`完整`／`部分`／`資料不足`），除非有更強理由才覆寫並說明原因 |
