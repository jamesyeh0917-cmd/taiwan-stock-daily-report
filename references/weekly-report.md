...
證據帳本每天 15–20 列、候選股每天 ~10–30 列、產業資金流每天 ~30-35 列，會無限增長。每月一次：
...
- **產業資金流**：`資料基準日` 早於 90 天前的列 → 整列刪除（不分狀態，全量刪除；此資料庫每天寫入全部 ~30-35 個產業供未來 backtest 用，不像證據帳本需要長期月度趨勢，過窗即無保留價值）。若 Notion 工具無法刪除 → 在「重點成分股」欄前綴 `[待壓縮]`，週報列出待清理數量。
- 若 Notion 工具無法刪除／封存 → 在該列「判讀」欄前綴 `[待壓縮]`，並在週報列出「本月待人工清理 N 列」。
- 每次壓縮在週報記錄「壓縮：證據帳本 -X 列、候選股 -Y 列、產業資金流 -Z 列」。
